(function(window, document){
    ...
    const Paginas = {
      ...
      // --- Objeto 'cadastro' CORRIGIDO e no lugar certo ---
      cadastro: {
        salvarCadastro(){
          const nome  = (document.getElementById('nome')?.value || '').trim();
          const email = (document.getElementById('email')?.value || '').trim();
          const senha = document.getElementById('senha')?.value || '';
          const confirmarSenha = document.getElementById('confirmar-senha')?.value || ''; // <-- ADICIONADO
   
          // Validação dos campos
          if(!nome || !email || !senha || !confirmarSenha){ // <-- MODIFICADO
          ...
          if(senha !== confirmarSenha){ // <-- ADICIONADO
            IU.mensagem('mensagem','As senhas não conferem.');
            return;
          }
          ...
          // limpar campos
          ['nome','email','senha','confirmar-senha'].forEach(id => { // <-- MODIFICADO
